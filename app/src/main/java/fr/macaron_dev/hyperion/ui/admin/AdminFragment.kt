...
import fr.macaron_dev.hyperion.activity.ProjectValidActivity
...
class AdminFragment : Fragment(), SwipeRefreshLayout.OnRefreshListener, SearchView.OnQueryTextListener, View.OnClickListener {
            ...
            if (!isLoading) {
                if (recyclerView.layoutManager !== null && (recyclerView.layoutManager as LinearLayoutManager).findLastCompletelyVisibleItemPosition() == projects.size -1) {
                    ...
                    CoroutineScope(Dispatchers.Default).launch {
                        page++
                        val search = root.findViewById<SearchView>(R.id.adminSearch).query.let {
                            if(it == ""){
                                null
                            }else{
                                it.toString()
                        ...
                        val res = api.getProjectToValid(page, search)
    ...
    override fun onCreateView(
        ...
        root.findViewById<RecyclerView>(R.id.admin_recycler).addOnScrollListener(this.ScrollListener())
        return root
    }

    override fun onResume() {
        super.onResume()
        swipe = root.findViewById(R.id.swipeAdmin)
        swipe.setOnRefreshListener(this)
        root.findViewById<SearchView>(R.id.adminSearch).setOnQueryTextListener(this)

        CoroutineScope(Dispatchers.Default).launch {
            val project = api.getProjectToValid(0, null)
    ...
    private suspend fun prepareAndDisplayRecycler(project: JSONArray) {
        if ((project[0] is JSONObject)) {
            ...
            withContext(Dispatchers.Main) {
                root.findViewById<ProgressBar>(R.id.admin_spinner).visibility = View.GONE
                root.findViewById<RecyclerView>(R.id.admin_recycler).apply {
    ...
    private fun adapterOnClick(project: Project) {
        val intent = Intent(root.context, ProjectValidActivity::class.java)
    ...
    override fun onQueryTextSubmit(query: String?): Boolean {
        isLoading = false
        CoroutineScope(Dispatchers.Default).launch {
            val res = api.getProjectToValid(0, query)
    ...
    override fun onClick(v: View?) {
        CoroutineScope(Dispatchers.Default).launch {
            val res = api.getProjectToValid(0, (v as SearchView).query.toString())
            if(res[0] is JSONObject){
                prepareAndDisplayRecycler(res)
            }
        }
